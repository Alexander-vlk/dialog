...
class HeroActionBlock(AutoDateMixin):
    ...
    short_description = models.CharField(
        max_length=400, verbose_name="Краткое описание", unique=True
    )
...
class CallToActionBlock(AutoDateMixin):
    ...
    action_text = models.CharField(
        max_length=200, verbose_name="Текст призыва", unique=True
    )
    short_description = models.CharField(
        max_length=400, verbose_name="Краткое описание", unique=True
    )
...
class SliderImage(AutoDateMixin):
    ...
    image = models.ImageField(
        upload_to="slider_images", null=True, blank=True, verbose_name="Изображение"
    )
