...
class DailyLogListView(ListView):
    ...
    def get_queryset(self):
        return (
            DailyLog.objects.filter(user=self.request.user)
            .annotate(
...
class WeeklyLogListView(ListView):
    ...
    def get_queryset(self):
        return (
            WeeklyLog.objects.filter(user=self.request.user)
            .annotate(
...
def monthly_log_list(request, monthly_log_id):
    ...
    monthly_logs = MonthlyLog.objects.filter(user=request.user).order_by("-created_at")[
        :10
    ]

    weekly_logs_by_month = WeeklyLog.objects.filter(
        user=request.user, monthly_log=monthly_log
    ).order_by("week_start")
    daily_logs_by_month = DailyLog.objects.filter(
        weekly_log__in=weekly_logs_by_month
    ).order_by("-date")
