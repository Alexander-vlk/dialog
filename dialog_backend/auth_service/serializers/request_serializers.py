...
class UserRegistrationRequestSerializer(serializers.ModelSerializer):
    ...
    repeat_password = serializers.CharField(
        write_only=True, required=True, max_length=128, help_text="Пароль (еще раз)"
    )

    remember = serializers.BooleanField(help_text="Запомнить меня")
    agreed_with_privacy = serializers.BooleanField(
        help_text="Согласен с политикой конфиденциальности"
    )
    ...
    def validate_birthday(self, birthday):
        ...
        if birthday <= datetime.date(1900, 1, 1):
            raise serializers.ValidationError(
                "День рождения не может быть меньше 1900 года"
            )
    ...
    def validate_diagnosis_date(self, diagnosis_date):
        ...
        if diagnosis_date <= datetime.date(1900, 1, 1):
            raise serializers.ValidationError(
                "Дата постановки диагноза не может быть меньше 1900 года"
            )

        if diagnosis_date > datetime.date.today():
            raise serializers.ValidationError(
                "Дата постановки диагноза не может быть в будущем"
            )
    ...
    def validate_agreed_with_privacy(self, agreed_with_privacy):
        ...
        if not agreed_with_privacy:
            raise serializers.ValidationError(
                "Пользователь не согласен с политикой конфиденциальности"
            )
    ...
    def validate(self, data):
        ...
        if data["birth_date"] > data["diagnosis_date"]:
            raise serializers.ValidationError(
                "Дата постановки диагноза не может быть меньше даты рождения"
            )

        if not data["diabetes_type"] and (
            data["diagnosis_date"] or data["treatment_type"]
        ):
            raise serializers.ValidationError(
                "Тип лечения не может быть установлен, если Вы не болеете диабетом"
            )
